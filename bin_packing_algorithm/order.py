...
import heapq
import numpy as np
from bin_packing_algorithm.entity import AlgorithmBox, Bin


class Order:
    def __init__(self, message_str):
        """
        根据输入算例信息创建订单类，包括起终点、取货点、箱子、货车等信息。
        : param message_str: 输入算例JSON文件的名称，字符串类型。
        """
        super().__init__()
        self.data = json.loads(message_str) # 读取算例信息
        self.bin = self._gen_bin() # 生成bin
        self.trucks_listdict = self.data["algorithmBaseParamDto"]["truckTypeDtoList"] # 生成货车信息
        self.file_name = self.data['estimateCode'] # 生成算例文件名称
        self.allplatform_listdict = self.data["algorithmBaseParamDto"]["platformDtoList"] # 生成取货点信息
        self.platform_num = len(self.allplatform_listdict) # 生成取货点数量信息
        self.boxes_listAlgorithmBox = self._gen_box_list() # 生成算法输入箱子类信息
        # 按取货点生成算法输入箱子类信息
        ...
        self.distanceMap_2dMatrix = self._gen_distance() # 生成距离矩阵信息
    ...
    def _gen_pack_pattern(self):
        """ 
        对每个platform构造1C-FLPs和1C-SP两种装载模式
        1C-FLP (full load pattern)
        1C-SP (split pattern) 
        """
    ...
    def _gen_packpattern_by_platform(self, platformCode_str, pack_by_v, truck_num_by_vORw):
        """
        为某个platform构造1C-FLPs和1C-SP两种装载模式。
        return: oneC_FLPs, oneC_SP均为dict类型。
        """
        truck_capacity = 0
        total_load = 0
...
if __name__ == "__main__":
    input_dir = 'D:\\Desktop\\python_code\\track1_submission\\inputs'
    output_dir = 'D:\\Desktop\\python_code\\track1_submission\\outputs'
    for file_name in os.listdir(input_dir):
        ...
        print(one_order.boxes_by_platform_dictlistAlgorithmBox['platform04'][0].all_directions)
